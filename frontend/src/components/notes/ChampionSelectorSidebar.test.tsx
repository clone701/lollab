...
describe('ChampionSelectorSidebar - 検索機能', () => {
    ...
    test('要件6.2: 入力に応じてリアルタイムでフィルタリングする', () => {
        ...
        // チャンピオン一覧セクションを取得
        const championListSection = screen.getByText('チャンピオン一覧').closest('section');
        ...
        expect(within(championListSection!).getByText('アーリ')).toBeInTheDocument();
    });
    ...
    test('要件6.3: 部分一致検索をサポートする', () => {
        ...
        expect(within(championListSection!).getByText('アーリ')).toBeInTheDocument();
        expect(within(championListSection!).getByText('アカリ')).toBeInTheDocument();
        expect(within(championListSection!).getByText('リー・シン')).toBeInTheDocument();
    });
    ...
    test('要件6.5: 検索結果が0件の場合、該当するチャンピオンが見つかりませんと表示する', () => {
        ...
        // エラーメッセージが表示される
        expect(screen.getByText('該当するチャンピオンが見つかりません')).toBeInTheDocument();
    });
